...
!  use amoeba_direct, only : AM_DIRECT_bcast
   ...
   use md_scheme, only: ischeme, ithermostat, therm_par, baro_stochastic
   use fft,only:column_fft_flag
! SOFT CORE
   use softcore, only : ifsc, scalpha, scbeta, scmask, dynlmb, &
        sceeorder, tishake, emil_sc, &
        ijarzynski, jarzynski_lambda_min, jarzynski_lambda_max
! end SOFT CORE
...
#  include "tgtmd.h"
   ...
   call mpi_bcast(baro_stochastic, 1, MPI_INTEGER, 0, commsander, ierr)
...
! end SOFT CORE
   ...
   call mpi_bcast(ijarzynski,1,MPI_INTEGER,0,commsander,ierr)
   call mpi_bcast(jarzynski_lambda_min,1,MPI_DOUBLE_PRECISION,0,commsander,ierr)
   call mpi_bcast(jarzynski_lambda_max,1,MPI_DOUBLE_PRECISION,0,commsander,ierr)
   call setup_mbar_parallel()
...
! constant Redox potential
   ...
   call setpar(nspm, ix(i70), ntp, ix(i02), xx(lmass), ntc, ntt)
